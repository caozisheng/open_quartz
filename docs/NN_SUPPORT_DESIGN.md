...
#### Phase 3: 背景移除（Background Removal）节点 ✅ 已完成 (v0.9.x)
...
#### Phase 4a: 深度估计 ✅ 已完成 (v0.9.x)

| 任务 | 模型 | 大小 | 输入 | 状态 |
|---|---|---|---|---|
| **深度估计** | MiDaS v2.1 small | 63MB | 256×256 固定 | ✅ 已实现 |

> Depth-Anything v2 small (分离 onnx_data 文件) 暂不支持，需要合并模型或外部 data 加载。

#### Phase 4b: JSON 输出任务 + 风格迁移（待实现）

> **需要新基础设施**：json 输出端口类型、关键点/骨骼可视化 overlay、结构化数据传递到 shader uniform。
> 作为独立任务后续跟进。
...
| **风格迁移** | Magenta Arbitrary | 88MB | 256×256 | ~50-80ms | `sampler2D` | 超出 30MB 选型线，待评估 |
