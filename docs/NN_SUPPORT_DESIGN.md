...
| chaiNNer 能力 | 重要性 | 难度 | 说明 |
|---|---|---|---|
| **Image Super-Resolution** | ⭐⭐⭐⭐⭐ | 中 | chaiNNer 的招牌功能，支持几十种架构 |
| **Background Removal** | ⭐⭐⭐⭐ | 中 | u2net/isnet，用户需求高 |
| **Model Introspection** | ⭐⭐⭐⭐ | 中 | 自动推断模型能力，降低用户门槛 |
| **Auto-Split Tiling** | ⭐⭐⭐ | 大 | 大图推理必需，浏览器环境 WebGPU 显存更受限 |
| **多推理框架** | ⭐⭐ | 特大 | PyTorch/NCNN/TensorRT；对浏览器无意义，Tauri native 才需要 |
| **Frame Interpolation** | ⭐⭐⭐ | 大 | RIFE 帧插值，视频处理场景 |
| **Model Conversion** | ⭐⭐ | 中 | PyTorch→ONNX 转换；Tauri native 才有价值 |
| **模型 FP16 支持** | ⭐⭐⭐ | 小 | ORT web 已支持，需要端口签名适配 |
...
| 模型 | 大小 | 说明 |
|---|---|---|
| **u2netp** | 4.7MB | 轻量版，效果可用 |
| **modnet** | 25MB | 轻量人像抠图，效果好 |
| **silueta** | 44MB | u2net 蒸馏版 |
| **u2net** | 176MB | 通用背景移除，效果最好 |
| **isnet-general-use** | 174MB | DIS 项目，边缘更精细 |
| **u2net_human_seg** | 176MB | 人像专用 |
| **rmbg-1.4 (BRIA)** | 176MB | 商用级效果 |

> **模型体积不是阻塞因素。** Tauri 桌面端模型随 app 打包或放在本地目录（`~/.openquartz/models/`），不走网络，无体积限制。纯 Web 场景首次下载大模型较慢，可用 IndexedDB 持久缓存。模型一旦加载到内存，大小只影响模型复杂度带来的推理耗时，与"能不能跑"无关。
...
参数:
  ...
  feather: float 0-10 (边缘羽化像素)
```

##### 实时管线价值

背景移除 + shader 后处理是一个极有价值的管线：

```
Camera → RemBg → 前景 alpha
                    ↓
        Shader (替换背景 / 模糊背景 / 色键)
                    ↓
              Renderer 输出
...
这是 chaiNNer 做不到的实时场景——chaiNNer 只能离线处理。

#### Phase 4: 更多视觉任务

| 任务 | 模型 | 输出类型 | 说明 | 优先级 |
|---|---|---|---|---|
| **深度估计** | MiDaS / Depth-Anything | `sampler2D` (depth map) | 深度图可驱动 shader 做 DOF / 3D 效果 | ⭐⭐⭐⭐ |
| **语义分割** | SegFormer / PP-LiteSeg | `sampler2D` (segmentation map) | 每像素分类，可做选区 | ⭐⭐⭐ |
| **风格迁移** | Arbitrary Style Transfer | `sampler2D` | 实时艺术风格 | ⭐⭐⭐ |
| **降噪** | NAFNet / SwinIR-DN | `sampler2D` | 图像降噪 | ⭐⭐⭐ |
| **人脸检测/关键点** | BlazeFace / MediaPipe Face | `roi` + `json` (landmarks) | 人脸效果的基础 | ⭐⭐⭐ |
| **人体姿态** | MoveNet / MediaPipe Pose | `json` (keypoints) | 体感交互 | ⭐⭐ |
| **OCR** | PaddleOCR / SVTR | `json` (text regions) | 文字识别 | ⭐⭐ |
| **修复/inpainting** | LaMa | `sampler2D` | 需要额外 mask 输入 | ⭐⭐ |

##### 深度估计 × shader 的典型管线

```
Camera/Image → Depth Estimation → depth map (sampler2D)
                                       ↓
                              Shader: DOF / parallax / fog
                                       ↓
                                  Renderer 输出

这个管线让用户用一个 ONNX 节点 + 一个 shader 就能实现"景深模糊"效果，
不需要写任何深度估计代码——这是 OpenQuartz 区别于 chaiNNer 的核心价值。
```
